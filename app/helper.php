...
if(!function_exists('typeQuery')){
    ...
    if(!function_exists('bestSellingBooks')){
        function bestSellingBooks(){
            $bestSellingBooks = Book::select('book.*', 'author.name', 'bookstats.price', 'bookstats.quantity', 'bookstats.sold')->join('author', 'author.auth_id', '=', 'book.auth_id')->join('bookstats', 'bookstats.ISBN', '=', 'book.ISBN')->join('category', 'category.ISBN', '=', 'book.ISBN')->orderBy('sold', 'desc')->limit(50)->get();
